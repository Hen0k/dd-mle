...
class SimpleModel(MLEModel):
    ...
    def _one_hot_encode(self, dataset: pd.DataFrame):
        ...
        df_ohe = pd.concat(
            [dataset.drop(categorical_columns, axis=1), dummies], axis=1)
    ...
    def fit(self, dataset: Any) -> dict:
        ...
        # Train-test split
        X_train, X_test, y_train, y_test = train_test_split(df_ohe.drop(
            "label", axis=1), dataset["label"], test_size=0.33, random_state=42)
